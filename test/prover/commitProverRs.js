...
class CommitProverRs {
  ...
  async generateWitness(circuitInput) {
    ...
    const witnessPath = path.join(__dirname, this.temporaryName + "_temp_witness.wtns");

    // Write circuit input to a temporary JSON file for witness generation
    const inputPath = path.join(__dirname, this.temporaryName + "_temp_input.json");
    fs.writeFileSync(
      inputPath,
  ...
  async rapidsnarkProve(zkeyPath, witnessPath) {
    const proofPath = path.join(__dirname, this.temporaryName + "_temp_proof.json");
    const publicPath = path.join(__dirname, this.temporaryName + "_temp_public.json");
