...
class CounterProver {
  ...
  async getProof(round_number, V2, participants2, P1_number, sig_C1, sig_C2, agg_pubkey, salt) {
    ...
    const P2 = new Array(n_participants).fill(0n).map((_, i) => (i >= participants2.length ? _ : participants2[i]));
    ...
    // console.log("Counter circuit input:", JSON.stringify(circuitInput));
